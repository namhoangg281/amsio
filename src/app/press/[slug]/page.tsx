...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const article = await fetchPublishedArticleBySlug("press", slug);
  if (!article) return { title: "Release not found — AMSIO International" };
...
export default async function PressArticlePage({ params }: PageProps) {
  ...
  return <ArticleDetail article={article} backHref="/press" />;
}
